...
const Countdown: React.FC<CountdownProps> = ({
    ...
    return (
        <div tw="flex flex-col justify-center items-center select-none">
            <h2 tw="uppercase font-semibold text-center mb-2 font-bold -mr-2 tracking-superwide">{title}</h2>
            <div tw="flex justify-center items-center">
                {digits.days && (
                    <Digit digit={timeLeft.days} initDuration={initDuration} title="Hari" animated={animated} />
                )}
                {digits.hours && (
                    <Digit digit={timeLeft.hours} initDuration={initDuration} title="Jam" animated={animated} />
                )}
                {digits.minutes && (
                    <Digit digit={timeLeft.minutes} initDuration={initDuration} title="Menit" animated={animated} />
                )}
                {digits.seconds && (
                    <Digit digit={timeLeft.seconds} initDuration={initDuration} title="Detik" animated={animated} />
                )}
            </div>
        </div>
    );
